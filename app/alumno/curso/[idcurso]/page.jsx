...
export default function CursoDetalleAlumno() {
  ...
  useEffect(() => {
    // Simulación de datos (puedes reemplazar con fetch real)
            ...
            <div>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {curso.asistencias.map((item) => (
                      <tr key={item.id} className='hover:bg-gray-50'>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                          {item.fecha}
                        </td>
                        <td className={`px-6 py-4 whitespace-nowrap text-sm font-medium ${item.estado === 'Falta' ? 'text-red-600' : 'text-green-600'}`}>
                          {item.estado}
                        </td>
                      </tr>
                    ))}
